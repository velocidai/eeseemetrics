...
import { INTERVAL_OPTIONS } from "../../../../uptime/monitors/components/dialog/GeneralTab";
import { MonitorActions } from "../../../../uptime/monitors/components/MonitorActions";
import { MonitorResponseTimeChart } from "../../../../uptime/monitors/components/MonitorResponseTimeChart";
import { StatusOrb } from "../../../../uptime/monitors/components/StatusOrb";
import { useUptimeStore } from "../../../../uptime/monitors/components/uptimeStore";
import { getHoursFromTimeRange } from "../../../../uptime/monitors/components/utils";
import { EventsTable } from "../../../../uptime/monitors/[monitorId]/components/EventsTable";
import { FilterBar } from "../../../../uptime/monitors/[monitorId]/components/FilterBar";
...
export default function SiteMonitorDetailPage() {
  const params = useParams<{ site: string; monitorId: string }>();
  const router = useRouter();
  const { site } = params;
  const monitorId = parseInt(params.monitorId);
  ...
  if (!monitor && !isLoadingMonitor) {
    return (
      <div className="p-2 md:p-4 max-w-[1300px] mx-auto text-center py-12">
        <h2 className="text-xl font-semibold mb-2">Monitor not found</h2>
        <Button onClick={() => router.push(`/${site}/uptime/monitors`)} variant="outline">
          Back to Monitors
        </Button>
      </div>
    );
  ...
  return (
    <div className="p-2 md:p-4 max-w-[1300px] mx-auto space-y-4">
      <Button
        variant="ghost"
        size="sm"
        onClick={() => router.push(`/${site}/uptime/monitors`)}
