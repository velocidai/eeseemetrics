...
1. Navigate to **Settings → Account** in your eesee dashboard
...
<Tab value="Pageview">
```bash
curl -X POST 'https://app.eeseemetrics.com/api/track' \
...
const trackEvent = async (eventData) => {
  const response = await fetch('https://app.eeseemetrics.com/api/track', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${process.env.eesee_API_KEY}`,
    },
    body: JSON.stringify({
      site_id: process.env.eesee_SITE_ID,
      ...eventData
    })
  });
...
def track_event(event_data):
    response = requests.post(
        'https://app.eeseemetrics.com/api/track',
        headers={
            'Content-Type': 'application/json',
            'Authorization': f"Bearer {os.getenv('eesee_API_KEY')}"
        },
        json={
            'site_id': os.getenv('eesee_SITE_ID'),
            **event_data
        }
    )
    return response.json()
...
func trackEvent(event TrackingEvent) error {
    event.SiteID = os.Getenv("eesee_SITE_ID")
    ...
    req.Header.Set("Authorization", "Bearer "+os.Getenv("eesee_API_KEY"))
...
public class eeseeTracker {
    private static final String API_URL = "https://app.eeseemetrics.com/api/track";
    private static final String API_KEY = System.getenv("eesee_API_KEY");
    private static final String SITE_ID = System.getenv("eesee_SITE_ID");
    ...
    public static void main(String[] args) throws Exception {
        eeseeTracker tracker = new eeseeTracker();
...
class eeseeTracker {
    private $apiUrl = 'https://app.eeseemetrics.com/api/track';
    private $apiKey;
    private $siteId;

    public function __construct() {
        $this->apiKey = getenv('eesee_API_KEY');
        $this->siteId = getenv('eesee_SITE_ID');
    }
...
class eeseeTracker
  API_URL = 'https://app.eeseemetrics.com/api/track'

  def initialize
    @api_key = ENV['eesee_API_KEY']
...
public class eeseeTracker
{
    ...
    private const string ApiUrl = "https://app.eeseemetrics.com/api/track";

    public eeseeTracker()
    {
        _httpClient = new HttpClient();
        _apiKey = Environment.GetEnvironmentVariable("eesee_API_KEY");
        _siteId = Environment.GetEnvironmentVariable("eesee_SITE_ID");
...
impl eeseeTracker {
    fn new() -> Self {
        Self {
            ...
            api_key: env::var("eesee_API_KEY").expect("eesee_API_KEY not set"),
            site_id: env::var("eesee_SITE_ID").expect("eesee_SITE_ID not set"),
            api_url: "https://app.eeseemetrics.com/api/track".to_string(),
        }
    }
...
class eeseeTracker {
    static let shared = eeseeTracker()

    private let apiUrl = "https://app.eeseemetrics.com/api/track"
    private let apiKey: String
    private let siteId: String

    init() {
        // Ensure API key and site ID are available
        guard let apiKey = ProcessInfo.processInfo.environment["eesee_API_KEY"],
              !apiKey.isEmpty,
              let siteId = ProcessInfo.processInfo.environment["eesee_SITE_ID"],
              !siteId.isEmpty else {
            fatalError("eesee_API_KEY and eesee_SITE_ID environment variables must be set")
        }
        self.apiKey = apiKey
        self.siteId = siteId
    }
    ...
    func trackEvent(_ eventData: [String: Any]) async throws {
        ...
        guard let httpResponse = response as? HTTPURLResponse,
            ...
            throw NSError(domain: "eeseeTracker", code: 1, userInfo: [NSLocalizedDescriptionKey: "Tracking failed"])
        }
    }
...
Task {
    // Track a pageview
    try await eeseeTracker.shared.trackEvent([
        "type": "pageview",
        "pathname": "/api/users",
        "hostname": "api.example.com",
        "page_title": "User API Endpoint"
...
class eeseeTracker(private val context: Context) {
    companion object {
        private const val API_URL = "https://app.eeseemetrics.com/api/track"
        private val JSON = "application/json; charset=utf-8".toMediaType()
    }

    private val client = OkHttpClient()
    private val apiKey = BuildConfig.eesee_API_KEY // Set in build.gradle
    private val siteId = BuildConfig.eesee_SITE_ID
...
class MainActivity : AppCompatActivity() {
    private val tracker = eeseeTracker(this)
...
class eeseeTracker {
  static final eeseeTracker _instance = eeseeTracker._internal();
  factory eeseeTracker() => _instance;
  eeseeTracker._internal();

  static const String _apiUrl = 'https://app.eeseemetrics.com/api/track';
  static const String _apiKey = String.fromEnvironment('eesee_API_KEY');
  static const String _siteId = String.fromEnvironment('eesee_SITE_ID');
...
<Callout type="info">
**Browser Tracking**: For standard browser-based tracking, use the [eesee Web SDK](/docs/sdks/web) instead. The HTTP API is designed for server-side applications, mobile apps, and custom integrations.
</Callout>
