---
title: Hugo
description: Integrate eesee Analytics with your Hugo static site using partial templates
---

import { Steps, Step } from 'fumadocs-ui/components/steps';

Integrating eesee Analytics with your Hugo static site involves adding the tracking script to your site's templates. The best way to do this is by creating a partial template for the script and including it in your site's footer.
...
Inside `layouts/partials/`, create a new file named `eesee-analytics.html` (or a similar name).

Paste your eesee tracking script into this new file:

```html
{{/* layouts/partials/eesee-analytics.html */}}
...
- Replace `https://app.eeseemetrics.com` and `YOUR_SITE_ID` with your actual details.
</Step>
...
Open your main base template or footer partial file. Just before the closing `</body>` tag, add the following line to include your eesee Analytics partial:

```html
{{/* ... other footer content ... */}}

{{ partial "eesee-analytics.html" . }}
...
<Steps>
<Step>
### Update `eesee-analytics.html`

Modify `layouts/partials/eesee-analytics.html` to use a site parameter:

```html
{{/* layouts/partials/eesee-analytics.html */}}
{{ if and (not .Site.IsServer) .Site.Params.eeseeSiteID }}
  <script async defer src="{{ .Site.Params.eeseeInstanceURL | default "https://app.eeseemetrics.com" }}/api/script.js" data-site-id="{{ .Site.Params.eeseeSiteID }}"></script>
{{ end }}
```
</Step>
...
```toml
[params]
  eeseeInstanceURL = "https://app.eeseemetrics.com" # Your eesee instance URL
  eeseeSiteID = "YOUR_SITE_ID" # Your eesee Site ID
```
...
That's it! eesee Analytics is now integrated with your Hugo site and will only track visits on your production deployment.