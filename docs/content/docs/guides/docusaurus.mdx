---
title: Docusaurus
description: Integrate eesee Analytics with your Docusaurus v2+ website
---
...
This guide explains how to integrate eesee with your Docusaurus v2+ website for analytics tracking. Docusaurus is a static site generator that builds a single-page application with React.

## How to Add eesee to Your Docusaurus Site

Docusaurus provides a configuration option to add scripts or tags to the `<head>` of every page, which is the recommended way to include eesee.

<Steps>
<Step>
### Retrieve Your Tracking Script Details

Navigate to your eesee dashboard to obtain your **Site ID**. The script source will be `https://app.eeseemetrics.com/api/script.js`.
</Step>
...
You can add the eesee script using the `scripts` array or the `headTags` array within your site configuration. Using `scripts` is often cleaner for simple script additions.
...
export default function MyCustomButton({ children, eventName, eventData }) {
  const handleClick = () => {
    if (typeof window !== 'undefined' && window.eesee) {
      window.eesee.event(eventName || 'docusaurus_custom_click', eventData || {});
    }
  };
...
```javascript filename="src/client-modules/eesee-events.js"
...
export function onRouteUpdate({location, previousLocation}) {
  ...
  if (typeof window !== 'undefined' && window.eesee && location.pathname !== previousLocation?.pathname) {
    // eesee should auto-track pageviews, but if needed:
  ...
  document.querySelectorAll('a[target="_blank"]').forEach(link => {
    link.addEventListener('click', () => {
      if (window.eesee) {
        window.eesee.event('external_link_click', { href: link.href });
      }
    });
  });
}
...
Always ensure `window.eesee` is available before calling its methods in client-side scripts.