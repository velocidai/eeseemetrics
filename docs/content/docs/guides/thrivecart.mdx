---
title: "ThriveCart"
description: "Integrate eesee Analytics with your ThriveCart checkout pages"
---
...
eesee can be integrated with ThriveCart to track cart views, purchases, and other conversion events on your checkout pages.

<Callout type="warn">
  ThriveCart strips out the standard `data-site-id` attribute from scripts. You must use the alternative installation method below for eesee to work correctly.
</Callout>

## Adding the Tracking Script

The standard eesee script won't work in ThriveCart because ThriveCart removes the `data-site-id` attribute. Instead, use this JavaScript snippet that dynamically creates the script element:
...
(function() {
  ...
  s.src = 'https://app.eeseemetrics.com/api/script.js';
  s.async = true;
...
Replace `YOUR_SITE_ID` with your actual eesee Site ID from your dashboard.
</Step>

<Step>
### Save and Test

Save your changes and visit your checkout page. Check your eesee dashboard to confirm pageviews are being tracked.
</Step>
</Steps>
...
(function () {
  ...
  var SENT_KEY = "eesee_" + EVENT_NAME + "_sent_v1";
  try {
  ...
  function isReady() {
    return window.eesee && typeof window.eesee.event === "function";
  }

  function send() {
    window.eesee.event(EVENT_NAME, PAYLOAD);
  }
  ...
  function ensureScriptLoaded() {
    ...
    if (document.querySelector('script[src="https://app.eeseemetrics.com/api/script.js"]')) return;
  ...
  // If eesee is already loaded, send immediately
  if (isReady()) return send();
...
If your ThriveCart is hosted on a subdomain (e.g., `sales.yourdomain.com`) while your main site is on `yourdomain.com`, eesee will track both as part of the same site as long as you use the same Site ID. This allows you to create funnels that track the user journey from your main site through to the ThriveCart checkout and purchase.

## Troubleshooting

### Events not appearing in eesee

1. **Check the script is loading**: Open your browser's Developer Tools (F12), go to the Network tab, and look for `script.js` being loaded from `app.eesee.io`.
2. **Check for JavaScript errors**: In the Console tab, look for any red error messages.
3. **Verify your Site ID**: Make sure you've replaced `YOUR_SITE_ID` with your actual Site ID from the eesee dashboard.
