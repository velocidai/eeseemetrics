---
title: WordPress
description: Integrate eesee Analytics with your WordPress site
---
...
You can integrate eesee into your WordPress site to track user behavior and gather analytics. There are several ways to do this depending on whether you're using a hosted WordPress.com site or a self-hosted WordPress.org installation.
...
3. Replace `YOUR_SITE_ID` with your actual Site ID from your eesee dashboard
...
Visit your site in a new browser tab, then check your eesee dashboard to confirm data is being received.
</Step>
</Steps>
...
add_action('wp_head', 'add_eesee_analytics');
```
...
3. Create a new folder called `eesee-analytics`
4. Inside that folder, create a file called `eesee-analytics.php` with this content:

```php
<?php
/**
 * Plugin Name: eesee Analytics
 * Description: Adds eesee Analytics tracking to your WordPress site
 * Version: 1.0
 */

function eesee_add_tracking_script() {
    ?>
    <script>
    (function() {
      var s = document.createElement('script');
...
add_action('wp_head', 'eesee_add_tracking_script');
```

5. Replace `YOUR_SITE_ID` with your actual Site ID
6. Go to **Plugins** in WordPress and activate "eesee Analytics"
</Tab>
</Tabs>
...
3. Search for `eesee` - you should see the script tag
...
Many WordPress caching plugins aggressively cache pages. After adding eesee:
...
2. **Exclude the script from optimization:**
   Some optimization plugins try to defer, combine, or minify scripts. If eesee isn't working, check these plugins:
   - **Autoptimize**: Add `app.eesee.io` to the exclusion list
...
Security plugins like Wordfence or Sucuri may block external scripts. Add `app.eesee.io` to your allowlist if tracking isn't working.
