---
title: "Shopify"
description: "Integrate eesee with your Shopify store to capture events, track conversions, and analyze user behavior."
---
...
eesee can be integrated with your Shopify store to capture events, track conversions, and analyze user behavior.

## How to Add eesee to Shopify

<Steps>
<Step>
### Retrieve Your Tracking Script

Navigate to your eesee dashboard to obtain your code snippet.
...
To get the most out of eesee with Shopify, you should track key e-commerce events. This typically involves adding small Liquid and JavaScript snippets to your theme files.
...
<script type="text/javascript">
  if (typeof window.eesee !== 'undefined' && {{ product | json }}) {
    const eeseeProduct = {{ product | json }};
    const eeseeCurrentVariant = {{ product.selected_or_first_available_variant | json }};

    window.eesee.event('view_item', {
      item_id: eeseeCurrentVariant.sku || eeseeCurrentVariant.id,
      item_name: eeseeProduct.title,
      item_variant: eeseeCurrentVariant.title === 'Default Title' ? null : eeseeCurrentVariant.title,
      price: parseFloat(eeseeCurrentVariant.price) / 100, // Price is in cents
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const trackAddToCart = function() {
    if (typeof window.eesee !== 'undefined' && {{ product | json }}) {
      ...
      window.eesee.event('add_to_cart', {
        item_id: eeseeCurrentVariant.sku || eeseeCurrentVariant.id,
        item_name: eeseeProduct.title,
        item_variant: eeseeCurrentVariant.title === 'Default Title' ? null : eeseeCurrentVariant.title,
        price: parseFloat(eeseeCurrentVariant.price) / 100,
        currency: {{ cart.currency.iso_code | json }},
...
Shopify's checkout process is on a separate domain (`checkout.shopify.com`), and direct script injection into the checkout pages (beyond the "Order status" page) is limited for most plans. However, eesee's script, if loaded on your main store, should persist session information.
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (checkoutButton && typeof window.eesee !== 'undefined' && {{ cart | json }}) {
    checkoutButton.addEventListener('click', function() {
      const eeseeCart = {{ cart | json }};
      let items = [];
      eeseeCart.items.forEach(function(item) {
        items.push({
      ...
      window.eesee.event('begin_checkout', {
        currency: eeseeCart.currency,
        value: parseFloat(eeseeCart.total_price) / 100,
        items: items
      });
    });
  }
...
<script type="text/javascript">
  if (typeof window.eesee !== 'undefined' && {{ checkout | json }}) {
    const eeseeCheckout = {{ checkout | json }};
    let items = [];
    eeseeCheckout.line_items.forEach(function(item) {
      items.push({
        item_id: item.sku || item.variant_id,
    ...
    window.eesee.event('purchase', {
      transaction_id: eeseeCheckout.order_number,
      value: parseFloat(eeseeCheckout.total_price),
      tax: parseFloat(eeseeCheckout.tax_price),
      shipping: parseFloat(eeseeCheckout.shipping_price),
      currency: eeseeCheckout.currency,
      items: items
    });
  }
</script>
...
*   **Currency Conversion**: Prices in Shopify Liquid objects are often in cents. Ensure you divide by 100 to get the decimal value for eesee.
...
*   **Shopify Customer Events**: For more robust server-side or client-side event tracking, especially for `add_to_cart` and `begin_checkout` without relying on DOM manipulation, explore [Shopify Customer Events](https://shopify.dev/docs/api/customer-events). This allows you to subscribe to events like "product added to cart" or "checkout started" and send data to eesee via a custom pixel or by triggering client-side JavaScript. This is a more advanced but often more reliable method.
*   **Testing**: Thoroughly test each event in your eesee dashboard after implementation. Use your browser's developer console to check for JavaScript errors.