...
eesee can automatically capture common user interactions without requiring any code changes. These features can be enabled in your [Site Settings](/docs/site-settings) under the "Auto Capture" section.
...
When enabled, eesee automatically tracks clicks on links that navigate to external domains.
...
You can add custom properties to auto-tracked button clicks using `data-eesee-prop-*` attributes:

```html
<button
  data-eesee-prop-category="pricing"
  data-eesee-prop-variant="blue"
>
  Buy Now
</button>
```
...
If you want to track a button with a custom event instead of the automatic `button_click`, add the `data-eesee-event` attribute:

```html
<!-- This button will fire a custom "signup" event, NOT a button_click -->
<button data-eesee-event="signup" data-eesee-prop-plan="pro">
  Sign Up
</button>
```
...
You can add custom properties using `data-eesee-prop-*` on the form element:

```html
<form
  id="signup"
  data-eesee-prop-form-type="registration"
  data-eesee-prop-variant="a"
>
  ...
</form>
```
...
You can add custom properties using `data-eesee-prop-*` on the input element:

```html
<input
  name="email"
  data-eesee-prop-field-category="contact"
/>

<select
  name="country"
  data-eesee-prop-section="demographics"
>
  ...
</select>
```
