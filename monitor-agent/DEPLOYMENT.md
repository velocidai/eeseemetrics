# Eesee Metrics Monitor Agent Deployment Guide

This guide covers deploying the Eesee Metrics Monitor Agent to VPS instances in different regions.
...
- Main Eesee Metrics server IP address for whitelisting
...
sudo mkdir -p /opt/eesee-metrics-monitor-agent
cd /opt/eesee-metrics-monitor-agent
...
wget https://raw.githubusercontent.com/your-repo/eesee-metrics/main/monitor-agent/docker-compose.yml
wget https://raw.githubusercontent.com/your-repo/eesee-metrics/main/monitor-agent/docker-compose.prod.yml
wget https://raw.githubusercontent.com/your-repo/eesee-metrics/main/monitor-agent/Caddyfile
...
sudo nano /opt/eesee-metrics-monitor-agent/.env

# Restart the service
sudo systemctl restart eesee-metrics-monitor-agent
```
...
Add the agent regions to your main Eesee Metrics server database:
...
3. **Main Server**: Check region health in the Eesee Metrics UI
...
# Systemd deployment
sudo journalctl -u eesee-metrics-monitor-agent -f
sudo systemctl status eesee-metrics-monitor-agent
```
...
cp /opt/eesee-metrics-monitor-agent/.env /opt/eesee-metrics-monitor-agent/.env.backup
```
